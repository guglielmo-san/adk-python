...
class TestA2aAgentExecutor:
  ...
  async def test_after_event_interceptors_receive_correct_arguments_and_can_modify_event(
    ...
    mock_interceptor.after_event = AsyncMock(side_effect=side_effect_after_event)
    mock_interceptor.before_agent_execute = None
    mock_interceptor.after_agent_execute = None

    # Update config with interceptor
    self.mock_config.execute_interceptors = [mock_interceptor]
    # Re-initialize executor with updated config - but we can just update the config in place if it's mutable
    ...
    # So updating self.mock_config properties should work as it is the same object reference.

    # Mock context
    self.mock_context.task_id = "task-1"
    self.mock_context.context_id = "ctx-1"
    # Ensure current_task is set so we skip the initial submitted event creation logic 
    ...
    passed_executor_context = call_args.args[0]
    passed_a2a_event = call_args.args[1]
    passed_adk_event = call_args.args[2]

    # These assertions verify the bug fix
    assert passed_a2a_event is a2a_event, f"Expected A2A event to be passed as 2nd arg, but got {passed_a2a_event}"
    assert passed_adk_event is adk_event, f"Expected ADK event to be passed as 3rd arg, but got {passed_adk_event}"

    # Verify that the modified event was enqueued
    # We check if enqueue_event was called with modified_a2a_event
    # Note: enqueue_event is called multiple times (working status, then our event, then final status)
    
    enqueued_events = [
        call[0][0] for call in self.mock_event_queue.enqueue_event.call_args_list
    ]
    assert modified_a2a_event in enqueued_events, "The modified event should have been enqueued"
